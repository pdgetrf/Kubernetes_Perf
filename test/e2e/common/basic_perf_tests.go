...
var _ = ginkgo.Describe("PodPerf", func() {
	...
	ginkgo.It("PodE2EStartLatency", func() {
		...
		createPodAndMeasureTime := func(nsIdx, podIdx int) {
			...
			lw := &cache.ListWatch{
				ListFunc: func(options metav1.ListOptions) (object runtime.Object, e error) {
					...
					return f[nsIdx].ClientSet.CoreV1().Pods(ns[nsIdx]).List(ctx, options)
				},
				WatchFunc: func(options metav1.ListOptions) (i watch.Interface, e error) {
					options.FieldSelector = fieldSelector
					options.TimeoutSeconds = &timeout
					return f[nsIdx].ClientSet.CoreV1().Pods(ns[nsIdx]).Watch(ctx, options)
				},
			}
			...
			tStamp1 := time.Now()
			for {
				p, err := f[nsIdx].ClientSet.CoreV1().Pods(ns[nsIdx]).Create(context.TODO(), tPod, metav1.CreateOptions{})
				if err == nil {
					pods[idx] = p
					break
				}
		...
		getPodAndMeasureTime := func(nsIdx, podIdx int) {
			idx := nsIdx*numPodsPerNs + podIdx
			tStamp1 := time.Now()
			_, err := podClient[nsIdx].Get(context.TODO(), pods[idx].Name, metav1.GetOptions{})
			tStamp2 := time.Now()
		...
		for i := 0; i < numNs; i++ {
			...
			tStamp1 := time.Now()
			listPods, err := podClient[i].List(context.TODO(), metav1.ListOptions{LabelSelector: selector.String()})
			tStamp2 := time.Now()
